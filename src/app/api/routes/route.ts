...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.id || !body.name || !body.description || !body.color || !Array.isArray(body.stops)) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: id, name, description, color, stops' },
        { status: 400 }
      );
    }
    ...
    for (const stop of body.stops) {
      if (!stop.name || !Array.isArray(stop.coordinates) || stop.coordinates.length !== 2) {
        return NextResponse.json(
          { success: false, error: 'Invalid stop structure. Each stop must have name and coordinates [lng, lat]' },
          { status: 400 }
        );
      }
    }
    ...
    if (existingRoute) {
      return NextResponse.json(
        { success: false, error: `Route with ID '${body.id}' already exists` },
        { status: 409 }
      );
    }
